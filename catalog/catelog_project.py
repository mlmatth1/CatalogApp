...
def fbconnect():
    ...
    '''
        ...
        the server token exchange we have to split the
        token first on commas and select the first index
        which gives us the key : value for the server access
        token then we split it on colons to pull out
        the actual token value and replace the remaining
...
def catalogJSON():
    ...
    categories = session.query(CatagoryItem).all()
...
def showCatagories():
    ...
    items = session.query(
        Catagories, CatagoryItem).\
        join(CatagoryItem, Catagories.id == CatagoryItem.category_id).\
...
def showCatagory(catagory_id):
    if 'username' not in login_session:
        ...
        items = session.query(
            CatagoryItem).filter_by(
                category_id=catagory_id).all()
...
@app.route(
    '/catagory/<int:catagory_id>/CatagoryItem/new/', methods=['GET', 'POST'])
def newCatagoryItem(catagory_id):
    ...
    if 'username' not in login_session:
        ...
        if request.method == 'POST':
            newItem = CatagoryItem(
                name=request.form['name'],
                description=request.form['description'],
                category_id=catagory.id, user_id=catagory.user_id)
            session.add(newItem)
            session.commit()
...
@app.route('/catagory/<int:catagory_id>/')
@app.route(
    '/catagory/<int:catagory_id>/ \
    CatagoryItem/<int:catagory_item_id>/description')
def showItemDescription(catagory_id, catagory_item_id):

    catagoryItemDescription = \
    ...
    if 'username' not in login_session or \
        ...
        return render_template(
            ...
            items=catagoryItemDescription, creator=creator)

""" Modify Description of Category Items """


@app.route(
    '/catagory/<int:catagory_id>/CatagoryItem/ \
